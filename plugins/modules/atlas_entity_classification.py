...
DOCUMENTATION = r'''
---
module: atlas_entity_classification
short_description: Create, update, and delete classifications for individual Apache Atlas entities
description:
  - Manage (create, update, delete) classifications for individual Apache Atlas entities.
...
class AtlasEntityClassification(AnsibleModule):
    ...
    def process(self):
        # TODO: Need to resolve issues with the underlying Atlas library resolves its parsing error(s)
        self.module.fail_json(msg='NOT SUPPORTED')
        
        entity = self._get_entity()
        if entity is None:
            self.module.fail_json(msg='Entity not found')
            
        existing = self._get_classifications(entity.entity.guid)
    ...
    def _get_classifications(self, guid):
        try:
            return self.atlas.entity.get_classifications(guid)
        except AtlasServiceException as err:
            error = re.search(CLIENT_ERROR_PATTERN, str(err))
            if error.group(2) == '200':
                return None
            else:
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            classifications=dict(required=True, type='list', elements='dict', contains=dict(
                entityGuid=dict(required=True, type='str', aliases=['entity_guid', 'entity'])
                #entityStatus
                #propagate
    ...
    result = AtlasEntityClassification(module)

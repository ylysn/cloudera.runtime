...
short_description: Gather information about Apache Ranger policies
description:
...
class RangerPolicyInfo(AnsibleModule):
    ...
    def process(self):
        if self.id is not None:
            self.policies = [self.ranger.get_policy_by_id(self.id)]
        elif self.service is not None and self.name is None:
            self.polices = self.ranger.get_policies_in_service(self.service)
        elif self.service is not None and self.name is not None:
            self.policies = [self.ranger.get_policy(self.service, self.name)]
        elif self.name is not None:
            self.policies = self.ranger.find_policies({ 'policyName': self.name })
        else:
            self.policies = self.ranger.find_policies()
...
def main():
    ...
    result = RangerPolicyInfo(module)
